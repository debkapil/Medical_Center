...
public class RegisterActivity extends AppCompatActivity {
    EditText edUsername,edFullName,edEmail,edPassword,edConfirmPassword;
    Button btn;
    TextView tv;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Database db= new Database(getApplicationContext(),"medicalCenter",null,1);

                if(username.length()==0||fullName.length()==0||email.length()==0||password.length()==0||confirmPassword.length()==0){
                    Toast.makeText(getApplicationContext(),"Please fill All details",Toast.LENGTH_SHORT).show();
                }
                else{
                    if(password.compareTo(confirmPassword)==0) {
                        if (isValid(password)) {
                            db.register(username,fullName,email,password);
